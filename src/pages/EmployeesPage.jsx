    ...
    <div className="page-card">
      <div className="page-header">
        ...
        <div className="toolbar-group">
          <button className="btn" type="button" onClick={onExportCsv}>{t(lang, 'downloadFilteredCsv')}</button>
          <button className="btn" type="button" onClick={onExportXlsx}>{t(lang, 'downloadFilteredXlsx')}</button>
        </div>
      </div>
      ...
      <div className="chart-card" style={{ marginBottom: 10 }}>
        ...
        <div className="badge-row">
          {Object.keys(visibleColumns).map((col) => (
            <label key={col} className="badge" style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
              <input
                type="checkbox"
      ...
      <div className="table-wrap" style={{ maxHeight: 520 }}>
        <table className="table">
          ...
          <tbody>
            {paged.items.map((row) => (
              <tr
                key={`${row.EmployeeNumber}-${row.ContractNumber}`}
      ...
      <div className="pagination">
        <div>
          {lang === 'ar' ? 'إجمالي النتائج' : 'Total Results'}: <strong>{formatNumber(paged.total)}</strong>
        </div>
        ...
        <div className="toolbar-group">
          ...
          <span className="badge">{page} / {paged.pages}</span>
          <button className="btn" type="button" onClick={() => setPage(Math.min(paged.pages, page + 1))}>
            {lang === 'ar' ? 'التالي' : 'Next'}
          </button>
        </div>
      </div>
